...
export default {
  data() {
    return {
      statusOptions: ['normal user(visitor)', 'editor(employee)', 'admin(manager)'],
    ...
    async getRoles() {
      getUserList().then(response => {
        ...
        for (var i = 0; i < response.data.user.length; i++) {
          if (this.TotalUser[i].role === 0) {
            this.TotalUser[i].role = 'visitor'
          } else if (this.TotalUser[i].role === 1) {
            this.TotalUser[i].role = 'manager'
          } else if (this.TotalUser[i].role === 2) {
            this.TotalUser[i].role = 'admin'
          }
        }
      }).catch(error => {
        console.log(error)
