...
const Places = ({ themeMode = 'light' }) => {
  ...
  function getFacilityDetail () {
    ...
    axios.post('/api/v1/facility/detail', postData).then((response) => {
      ...
      setCenter([response.data.data.facility.facility.long, response.data.data.facility.facility.lat]);
      // set place
      ...
      <Container>
        <Grid container>
          <Grid item xs={12} md={4} sm={12} sx={{ borderRadius: 5, border: 1, borderColor: 'divider' }}>
            <Box m={2}>
              <div style={{ width: '100%', height: 300, margin: 'auto', marginTop: 10 }}>
                <Typography
                  sx={{
                ...
                <APILoader akey="9f9859273ddca3aaa524fc161f19723d">
                  <Map zoom={13}
                    center={center}
                  >
