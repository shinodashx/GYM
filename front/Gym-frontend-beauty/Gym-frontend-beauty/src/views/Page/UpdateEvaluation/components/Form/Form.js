...
const Form = ({evaluation}) => {
  ...
  const [images, setImages] = React.useState((evaluation.images === '' ? []:[].concat((evaluation.images).split(','))));
  const imagesRef = React.useRef(images);
  const [videos, setVideos] = React.useState((evaluation.videos === '' ? []:[].concat((evaluation.videos).split(','))));
  ...
  const initialValues = {
    ...
    anonymous: (evaluation.anonymous !== 0),
  ...
  const onSubmit = async (values) => {
    try {
      ...
      const res = await axios.post('/api/v1/evaluation/update', {
        id: evaluation.id,
        facility_id: evaluation.facilityId,
        score: values.score,
        is_anonymous: anon,
