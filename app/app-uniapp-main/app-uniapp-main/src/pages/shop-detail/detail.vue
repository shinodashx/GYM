  ...
  <view class="main">
    <view v-if="isLoading === false">
          ...
          <view>
            <view class="page-body">
              <view class="page-section page-section-gap">
                <map style="width: 100%; height: 300px;" :latitude="this.latitude" :longitude="this.longitude"
                  :markers="this.covers">
                </map>
              </view>
            </view>
      ...
      <view class="places" v-if="facility.places.length !== 0">
        ...
        <view class="number">
          <u-text size="35" align="right" color="black" :text="facility.places.length + 'Goods'"></u-text>
        </view>
        <!-- loading -->
        <u-loading-icon :show="isLoading"></u-loading-icon>
        <!--                <u-cell-group v-for="item in facility.places" :key="item.id">-->
        <!--                    <u-cell :name="item.id" :title="item.name" :value="item.cost" :isLink="true" @click="placeDetail"-->
            ...
            <u-row>
              <u-col span="3">
                <view class="place-image">
                  <u-image class="place-image1" :src="facility.facility.images[0].url" width="200" height="120"
                           border-redius="20rpx"></u-image>
                </view>
              </u-col>
              <u-col span="1.5">
              </u-col>
              <u-col span="4.5">
                ...
                <u-row span="24">
                  <u-col>
                    <u-text size="20" color="grey" background-color="white"
                            :text="'Free cancel more than 2 hours before start'"></u-text>
                  </u-col>
                </u-row>
              </u-col>
              <u-col span="0.5">
              ...
              <u-col span="2.5">
                <u-col>
                  <u-text size="36" color="black" background-color="white" margin-left="20rpx"
                          :text="item.cost + '$'"></u-text>
                </u-col>
                <u-col>
                ...
                <u-col>
                  <view class="book-button">
                    ...
                    <view class="book-content" @click="placeDetail(item.id)">
                      <u-text size="35" color="white" bgcolor="blue" margin-left="20rpx" :text="'BUY'"></u-text>
                    </view>
                    <!--                            </u-row>-->
                    <!--                            <u-row>-->
                    <view class="book-content-ol" @click="placeDetail(item.id)">
                      <view style="margin-left: 8rpx">
                        <u-text class="book-content-ol" size="20" color="#004098" margin-left="20rpx"
                                :text="'pay'"></u-text>
                      </view>
                    ...
                    <!--                          <u-button type="primary" :text="'Book'" @click="placeDetail(item.id)"></u-button>-->
                  </view>
      ...
      <view>
        ...
        <u-popup :show="show" :round="10" mode="bottom" @close="close" class="item">
          <view class="select-bar">
            <template>
              <u-subsection bold fontSize="22" :list="orderTypeList" :current="curNow" :animation="true" @change="changeType"></u-subsection>
            </template>
          </view>
          <view class="modal-view" v-if="curNow === 0">
            <u-form label-position="left" label-width="200">
              ...
              <u-form-item borderBottom label="Start Time">
                ...
                <u-datetime-picker :filter="filter" :maxDate="maxDate" :minDate="1682350200000" :show="startTimeShow"
                                   v-model="form.startTime" mode="datetime" closeOnClickOverlay @confirm="confirmTime" @cancel="cancel"
                                   @change="change" @close="closeTime">
                </u-datetime-picker>
              </u-form-item>
              <u-form-item borderBottom label="End Time">
                ...
                <u-datetime-picker :filter="filter"
                                   ...
                                   :show="endTimeShow" v-model="form.endTime" mode="datetime" closeOnClickOverlay @confirm="confirmTime"
                                   @cancel="cancel" @change="change" @close="closeTime">
                </u-datetime-picker>
              </u-form-item>
          ...
          <view class="modal-view" v-if="curNow === 1">
            <u-form label-position="left" label-width="200">
              <u-form-item borderBottom label="Facility">
                <u-text size="35" color="grey" :text="facility.facility.name"></u-text>
              </u-form-item>
              ...
              <!--                      <u-form-item borderBottom label="End Time">-->
              <u-form-item label="weekCount" borderBottom>
                <u-input v-model="weekCount" placeholder="Please select payment type"></u-input>
                <u-icon slot="right" name="arrow-right"></u-icon>
              </u-form-item>
              <!--                      </u-form-item>-->
              <u-form-item borderBottom label="Total Cost">
                <u-text size="35" color="grey" :text="calculateTotalCostForWeeklyOrder()"></u-text>
              </u-form-item>
              <!-- btn -->
              <u-form-item>
                <u-button type="primary" size="large" @click="confirmRegularOrder">Submit</u-button>
              </u-form-item>
            </u-form>
          </view>
        </u-popup>
    ...
    <view class="page_liebiao1">
      ...
      <view class="yonghu" v-for="(item,index) in pro">
        ...
        <view style="margin-top: 5px;margin-left: 15px;">{{item.evaluation.description}}</view>
        <view class="tupian">
          <u-album :urls="item.evaluation.images" multipleSize="100px"></u-album>
        </view>
...
import { createOrder, createRegularOrder } from '@/api/order';
...
export default {
  ...
  methods: {
    ...
    calculateTotalCostForWeeklyOrder() {
      let hours = this.calculateTotalTime()
      if (hours == 'Select Time First') {
        return 'Select Time First'
      }
      hours = parseInt(hours)
      return hours * this.modalData.cost * this.weekCount + ' RMB'
    },
    async confirmOrder() {
      let data = {
    ...
    async confirmRegularOrder() {
      let data = {
        placeId: this.form.placeId,
        session_start_time: this.form.startTime,
        session_end_time: this.form.endTime,
        week_count: this.weekCount,
        start_day: this.form.startTime,
      }
      await createRegularOrder(data).then(
          res => {
            let orderCode = res.data.orderCode
            uni.navigateTo({
              url: '/pages/payment/payment?code=' + orderCode
      ...
      await getFacilityEvaluations(this.facilityId).then(
          res => {
            ...
            for (let i = 0; i < this.pro.length; i++) {
              this.pro[i].urls = []
              // split the images by ,
              this.pro[i].evaluation.images = this.pro[i].evaluation.images.split(',')
              for(let j = 0; j < this.pro[i].evaluation.images.length; j++) {
                this.pro[i].evaluation.images[j] = 'http://101.42.160.53:16800/' + this.pro[i].evaluation.images[j]
                this.pro[i].urls.push(this.pro[i].evaluation.images[j])
              }
              console.log(this.pro[i].evaluation.images)
            }
            this.isLoading = false
          }
      ).catch(err => {
