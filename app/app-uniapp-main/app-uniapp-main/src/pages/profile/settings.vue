<template>
  <view class="main" v-if="isLogin && isLoading === false">
        ...
        <u-row>
          <u-col span="3">
            <view class="img-profile">
              <image :src="user.avatar" mode="" mode="" @click="changeAvatar">
    ...
    <view class="actions">
      <u-cell-group>
        <u-cell icon="../../static/setting.svg" title="Change Password" isLink @click="toSetPassword"></u-cell>
      </u-cell-group>
    </view>
    ...
    <view class="actions">
      <u-cell-group>
        ...
        <u-cell icon="../../static/card.svg" title="Email" @click="changeAvatar" isLink></u-cell>
        <u-cell icon="../../static/card.svg" title="Email" @click="toSetEmail" isLink></u-cell>
        <u-cell icon="../../static/order.svg" title="Phone" @click="toSetPhone" isLink></u-cell>
  ...
  methods: {
    ...
    async getUserProfile() {
      await getProfile().then(res => {
        ...
        this.user.username = 'name: ' + this.user.username
        ...
        this.genderName = 'gender: ' + this.genderName
      }).catch(err => {
        console.log(err)
      })
    },
    ...
    toSetting() {
      uni.navigateTo({
        url: '/pages/profile/setusername'
      })
    },
    toOrder() {
      uni.navigateTo({
        url: '/pages/profile/setemail'
      })
    },
    toWallet() {
      uni.navigateTo({
        url: '/pages/profile/setphone'
      })
    },
    toSettings(){
      uni.navigateTo({
    ...
    uploadFilePromise(url) {
      return new Promise((resolve, reject) => {
        let a = uni.uploadFile({
          url: 'http://101.42.160.53:16800/api/v1/upload/avatar', // 仅为示例，非真实的接口地址
          filePath: url,
          name: 'file',
          success: (res) => {
            setTimeout(() => {
