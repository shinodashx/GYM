...
import OrderCard from '../../components/order-card-sch';
export default {
  name: "schedule",
  components: {
    UniCalendar,
  ...
  methods: {
    async getOrdersList() {
      ...
      let item1;
      let tmp1 = []
      for(item1 in this.orders){
        let date = this.orders[item1].order.startTime.split(" ")[0]
        tmp1.push({date: date})
      }
      this.selected = tmp1
      this.orders.sort((a, b) => {
        return new Date(a.order.startTime) - new Date(b.order.startTime);
      });
      // reverse the order
